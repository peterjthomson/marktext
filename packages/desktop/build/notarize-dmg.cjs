...
module.exports = async function notarizeDmg (buildResult) {
  ...
  // The update-info yml is written after this hook returns, so its DMG
  // checksums are refreshed by build/refresh-update-info.cjs as a post-build
  // step in the build:mac* scripts.
  return []
}
